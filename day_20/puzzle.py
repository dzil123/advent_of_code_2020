...
class Tile:
    ...
    def _create(cls, north, south, east, west):
        return cls(
            north=tuple(north), south=tuple(south), east=tuple(east), west=tuple(west)
        )
...
def assemble_tiles(tiles):
    ...
    random_tile_id = random.choice(
        list(adjacent.keys())
    )  # pick random tile to be at the center
    random_tile_orientation = random.choice(
        list(adjacent[random_tile_id].keys())
    ).tile  # and random orientation

    image = (
        {}
    )  # 2d array but sparse, dict[position(row(int), column(int)), OrientedTile]
    current_pos = (0, 0)
    ...
    while queue:
        ...
        next_tiles = {
            side.side: touching
            for side, touching in adjacent[current_tile.id].items()
            if side.tile == current_tile
        }
        print(next_tiles)
...
def part2(data):
    ...
    filled_image = {
        pos: rotate_full_tile(trimmed_raw_tiles[tile.id], tile.orientation)
        for pos, tile in image.items()
    }

    final_image = [
        [
            filled_image[low[0] + row // size[0], low[1] + col // size[1]][
                row % size[0]
            ][col % size[1]]
            for col in range(size[1] * extents[1])
        ]
        for row in range(size[0] * extents[0])
    ]
